        ...
        for idx, val_data in enumerate(dataloader):
            ...
            if self.bit == 0:
                ...
                if 'gt' in visuals:
                    ...
                    del self.gt
                # sr_img = (sr_img * 65535).clip(0, 65535).astype(np.uint16)
            else:
                temp_img_path = osp.join(self.opt['path']['visualization'], 'temp.tif')
                ...
                if self.opt['is_train']:
                    ...
                    if self.opt['val']['suffix']:
                        save_img_path = osp.join(self.opt['path']['visualization'], 'test',
                                                 ...
                                                 f"x{self.opt['scale']}", f'{img_name}_{self.opt["name"]}.tif')

                    else:
                        save_img_path = osp.join(self.opt['path']['visualization'], 'test',
                                                 self.opt['task'], dataset_name,
                                                 f"x{self.opt['scale']}", f'{img_name}.tif')
                # cv2.imwrite(save_img_path,(sr_img * 65535).astype('uint16'))
