...
class PyNETv2(nn.Module):
    def __init__(self, upscale: int, num_in_ch: int, num_out_ch: int, task: str,
        ...
        # -> Output: Level 3
        self.conv_l3_out = ConvLayer(128, 3 * k * k, 1, 1, relu=False,
    ...
    def level_3(self, conv_l3_d9):
        ...
        conv_l3_out = self.suffle1(conv_l3_out)
        output_l3 = torch.tanh(conv_l3_out) * 0.58 + 0.5

        return output_l3

    def level_2(self, conv_l2_d16):
        conv_l2_out = self.conv_l2_out(conv_l2_d16)
        conv_l2_out = self.suffle2(conv_l2_out)
    ...
    def forward(self, x):
        ...
        if self.level <= 3:
            output_l3 = self.level_3(conv_l3_d9)
        if self.level <= 2:
            output_l2 = self.level_2(conv_l2_d16)
        if self.level <= 1:
