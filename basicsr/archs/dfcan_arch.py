...
class DFCAN(nn.Module):
    def __init__(self, upscale: int, num_in_ch: int, num_out_ch: int, task: str, size_psc=128): #size_psc：crop_size input_shape：depth
        super(DFCAN, self).__init__()
        self.input=nn.Sequential(nn.Conv2d(num_in_ch, 64, kernel_size=3, stride=1, padding=1),
                                       nn.GELU(),)
        n_ResGroup=4
        ResGroups=[]
        ...
        self.conv_gelu=nn.Sequential(nn.Conv2d(64, 64*(upscale ** 2), kernel_size=3, stride=1, padding=1),
                                       nn.GELU())
        self.pixel_shuffle = nn.PixelShuffle(upscale)
        self.conv_sigmoid=nn.Sequential(nn.Conv2d(64, num_out_ch, kernel_size=3, stride=1, padding=1),
                                       nn.Sigmoid())
