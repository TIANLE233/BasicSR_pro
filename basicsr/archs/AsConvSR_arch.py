...
class AsConvSR(nn.Module):
    def __init__(self, planes: int, upscale: int, num_in_ch: int, num_out_ch: int, task: str, 
                 scale_factor: int=2, device=torch.device('cuda')):
        super(AsConvSR, self).__init__()
        self.scale_factor = scale_factor
        
        self.pixelUnShuffle = nn.PixelUnshuffle(upscale)
        self.conv1 = nn.Conv2d(num_in_ch * (upscale**2), planes, kernel_size=3, stride=1, padding=1)
        self.assemble = AssembledBlock(planes, planes, E=3, kernel_size=3, stride=1, padding=1, device=device)
        self.conv2 = nn.Conv2d(planes, num_in_ch * ((2 * upscale)**2), kernel_size=3, stride=1, padding=1)
        self.pixelShuffle1 = nn.PixelShuffle(upscale)
        self.conv3 = nn.Conv2d(num_in_ch * (upscale**2), num_out_ch * (upscale**2), kernel_size=3, stride=1, padding=1)
        self.pixelShuffle2 = nn.PixelShuffle(upscale)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out5 = self.pixelShuffle1(out4)
    
        x = torch.cat((x, x, x, x), dim=1)
        out6 = torch.add(out5, x)

        out7 = self.conv3(out6)
        out8 = self.pixelShuffle2(out7)
